...
# MATA: trip matching over a downloaded week of the poller archive, and its tests
gh release download archive-mata-2026-W39 --dir data/cache/mata/2026-W39
Rscript pipelines/mata/run.R --archive data/cache/mata/2026-W39 [--min-window-coverage 0]
...
- `arcgis.R`: `arcgis_json(req)` performs an ArcGIS REST query and retries both dropped connections and ArcGIS errors returned inside an HTTP 200 body. Use it for every ArcGIS fetch (311, permits, parcels; D25).
...
**Golden files.** `pipelines/311/tests/golden/` and `pipelines/permits/tests/golden/` hold frozen samples of real data and the metrics computed from them. The permits one also freezes its parcel counts, so a parcel refresh does not break it. Rebuild with `Rscript pipelines/<name>/tests/build_golden.R <raw.rds>` **only** when a spec version changes, and say why in the commit message. A golden-test failure is otherwise a regression. Food safety and MATA get golden files once real data exist (the H11 export; a well-covered month of MATA).
